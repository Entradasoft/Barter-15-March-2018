                    ...
                    duration:(NSTimeInterval)duration
{
    ...
    if( note )
    {
        [note setVisible:NO animated:NO completion:nil];
        //note = nil;
        
    }
    
    note = [[CSNotificationView alloc] initWithParentViewController:viewController];
    NSLog(@"Tint Color : %@",tintColor);
    note.tintColor = tintColor;
    note.textLabel.font= [UIFont fontWithName:@"TrebuchetMS-Bold" size:16];
    note.image = image;
    note.textLabel.text = message;
    
    NSInteger flgTmp=0;
    NSString *string = message;
    if ([string rangeOfString:@"success"].location == NSNotFound&&[string rangeOfString:@"Success"].location == NSNotFound)
    {
        flgTmp=0;
...
+(void)removeNotification:(UIViewController*)viewController
{
    NSLog(@"##################");
    if( note )
    {
        ...
        //Content views
        {
            //textLabel
            {
                if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
                    ...
                    _textLabel.font = [UIFont fontWithDescriptor:textLabelFontDescriptor size:20.0f];
                    _textLabel.adjustsFontSizeToFitWidth = YES;
                    
                    [self addSubview:_textLabel];
                    NSLog(@" Lines : %ld",(long)_textLabel.numberOfLines);
                }
                else
                {
                _textLabel = [[UILabel alloc] init];
                ...
                _textLabel.textAlignment = NSTextAlignmentCenter;
                UIFontDescriptor* textLabelFontDescriptor = [UIFontDescriptor preferredFontDescriptorWithTextStyle:UIFontTextStyleBody];
                _textLabel.font = [UIFont fontWithDescriptor:textLabelFontDescriptor size:12.0f];
                _textLabel.adjustsFontSizeToFitWidth = YES;
...
- (void)updateConstraints
{
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[blurView]|" options:0 metrics:nil views:bindings]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(-1)-[blurView]-(-1)-|" options:0 metrics:nil views:bindings]];
    CGFloat symbolViewWidth;
    CGFloat symbolViewHeight;
    if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)
    {
        symbolViewWidth = self.symbolView.tag != kCSNotificationViewEmptySymbolViewTag ?
        kCSNotificationViewSymbolViewSidelength_Ipad : 0.0f;
        symbolViewHeight = kCSNotificationViewSymbolViewSidelength_Ipad;
    }
    else
    {
     symbolViewWidth = self.symbolView.tag != kCSNotificationViewEmptySymbolViewTag ?
                                kCSNotificationViewSymbolViewSidelength : 0.0f;
     symbolViewHeight = kCSNotificationViewSymbolViewSidelength;
    }
...
- (CGRect)visibleFrame
{
    ...
    if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)
    {
        displayFrame = CGRectMake(0, 0, fabs(transformedSize.width),
                                  kCSNotificationViewHeight_Ipad + topLayoutGuideLength);
    }else
    {
        displayFrame = CGRectMake(0, 0, fabs(transformedSize.width),
...
- (CGRect)hiddenFrame
{
    ...
    if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)
    {
     offscreenFrame = CGRectMake(0, -kCSNotificationViewHeight_Ipad - topLayoutGuideLength,
                                       fabs(transformedSize.width),
                                       kCSNotificationViewHeight_Ipad + topLayoutGuideLength);
    }
    else{
        offscreenFrame = CGRectMake(0, -kCSNotificationViewHeight - topLayoutGuideLength,
                                    fabs(transformedSize.width),
                                    kCSNotificationViewHeight + topLayoutGuideLength);
    }
